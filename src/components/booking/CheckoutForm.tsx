...
export default function CheckoutForm({ quote, requireGuestNames }: { quote: BookingQuote; requireGuestNames: boolean }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok || !result.booking_id || !result.order_token) throw new Error(result.error || "The payment handoff could not be prepared.");
      router.push(`/payment/${result.booking_id}?token=${encodeURIComponent(result.order_token)}&method=${paymentMethod}`);
    } catch (reason) {
