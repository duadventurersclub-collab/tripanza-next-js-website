...
export type TourCardData = {
  ...
  details: {
    ...
    pricing: {
      quad: { display: string } | null;
      triple: { display: string } | null;
      twin: { display: string } | null;
    };
    seats_left?: string;
    cashback?: string;
    is_trending?: boolean;
...
function subscribeToUrlSearch(callback: () => void) {
...
function getUrlSearch() {
  ...
  return params.get("search") || params.get("destination") || "";
}

export default function ToursClient({ tours, total }: ToursClientProps) {
  ...
  const searchQuery = appliedSearch ?? urlSearch;
  const searchInput = draftSearch ?? searchQuery;
  ...
  function applySearch(value: string) {
    ...
    if (nextQuery) url.searchParams.set("search", nextQuery);
            ...
            {filterChips.map((chip) => {
              ...
              return (
                <button
                  type="button"
                  key={chip.label}
                  onClick={() => applySearch(chip.value)}
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Showing {visibleTours.length} of {searchQuery ? filteredTours.length : total} verified community tours
            </p>
          </div>
        ...
        {visibleTours.length < filteredTours.length && (
          <div className="mt-10 flex justify-center">
            <button
              ...
              className="rounded-xl border border-slate-300 bg-white px-6 py-3 text-sm font-bold text-slate-800 shadow-sm transition hover:border-emerald-500 hover:text-emerald-700"
            >
              Load more trips
            </button>
          </div>
